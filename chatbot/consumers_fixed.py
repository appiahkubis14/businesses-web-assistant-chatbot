...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_chat_message(self, message_data):
        ...
        try:
            ...
            # Get AI response
            ai_response = await self.get_ai_response(conversation, user_message)
            
            # Save AI response
            ai_msg = await self.save_message(conversation, 'assistant', ai_response)
            
            # Notify dashboard about AI response
            await self.notify_dashboard_new_message(conversation, ai_msg)
    ...
    @database_sync_to_async
    def get_ai_response(self, conversation, user_message):
        """Get AI response using the chatbot service"""
        try:
            chatbot_service = ChatbotService(conversation.website)
            return chatbot_service.generate_response(user_message, conversation)
        except Exception as e:
